...
import appframe.appframe.widget.swiperefresh.SwipeRefreshXMyfansAdapater;
...
public class MyfansActivity extends BaseActivity implements View.OnClickListener {
    ...
    SwipeRefreshXMyfansAdapater adapater;
    int Page = 1;
    ...
    protected void onResume() {
        super.onResume();
        initdata();
        MobclickAgent.onPageStart("我的粉丝页"); //统计页面(仅有Activity的应用中SDK自动调用，不需要单独写。"SplashScreen"为页面名称，可自定义)
    ...
    public void initdata()
    {
        ...
        Http.request(MyfansActivity.this, API.GETALLFANS, new Object[]{Http.getURL(map)}, new Http.RequestListener<List<UserDetail>>() {
            @Override
            public void onSuccess(List<UserDetail> result) {
                super.onSuccess(result);
                progress_bar.setVisibility(View.GONE);
                adapater = new SwipeRefreshXMyfansAdapater(MyfansActivity.this, result);
    ...
    protected void onPause() {
        super.onPause();
        MobclickAgent.onPageEnd("我的粉丝页"); // （仅有Activity的应用中SDK自动调用，不需要单独写）保证 onPageEnd 在onPause 之前调用,因为 onPause 中会保存信息。"SplashScreen"为页面名称，可自定义
